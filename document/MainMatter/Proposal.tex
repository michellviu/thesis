...
\begin{itemize}
    \item \textbf{RF-01: Gestión de Imágenes Dermatológicas}: El sistema debe permitir la carga de imágenes clínicas y dermatoscópicas, extrayendo y validando rigurosamente los 
    metadatos asociados.
    \item \textbf{RF-02: Gestión de Metadatos Clínicos}: El sistema debe soportar un esquema de metadatos 
    ...
    institucional.
    \item \textbf{RF-03: Control de Acceso y Visibilidad}: Debe ser posible clasificar las imágenes como públicas 
    o privadas. Las imágenes privadas estarán restringidas a usuarios con roles administrativos.
    \item \textbf{RF-04: Explotación Analítica y Estadística}: Se requiere que el sistema agrupe, filtre y tabule 
    ...
    \item \textbf{RF-05: Descarga Masiva}: Debe habilitarse un mecanismo para la descarga de galerías de imágenes 
...
\begin{itemize}
    \item \textbf{RNF-01: Autenticación Robusta}: Implementación de \textit{tokens} de acceso (JWT)~\cite{rfc7519_jwt} y protección de las 
    ...
    \item \textbf{RNF-02: Protección de Puntos de Entrada}: Validación estricta a nivel de infraestructura para 
    evitar manipulación de parámetros de negocio a través de validadores robustos.
    \item \textbf{RNF-03: Gestión de Datos Sensibles}: El esquema de identidad debe almacenar las credenciales 
...
\begin{itemize}
    \item \textbf{RNF-04: Interfaz Intuitiva y Responsiva}: La interfaz web debe adaptarse fluidamente a las 
...
\begin{itemize}
    \item \textbf{RNF-05: Persistencia Híbrida}: La arquitectura debe separar lógicamente el almacenamiento de 
...
\begin{itemize}
    \item \textbf{RE-01: Entorno de Ejecución}: \textit{Runtime} multiplataforma compatible con distribuciones 
    ...
    \item \textbf{RE-02: Motor de Bases de Datos}: Un Sistema Gestor de Base de Datos (SGBD) relacional robusto con características avanzadas 
    y alto rendimiento, estableciéndose como objetivo PostgreSQL.
    \item \textbf{RE-03: Servicios Externos}: Un servidor de Protocolo Simple de Transferencia de Correo (SMTP) para la emisión de notificaciones automatizadas y 
...
La elección de esta arquitectura por capas, fuertemente inspirada en los principios de la Arquitectura 
Limpia (\textit{Clean Architecture})~\cite{martin_clean_architecture}, se justifica por la necesidad de 
garantizar la mantenibilidad y escalabilidad del sistema a largo plazo. En el dominio médico, resulta 
fundamental que las reglas de negocio y las validaciones clínicas permanezcan estrictamente aisladas de 
los detalles de infraestructura o de las tecnologías de presentación. Esta separación de responsabilidades 
no solo facilita la realización de pruebas de código, sino que también permite que componentes tecnológicos 
externos (como el motor de base de datos o el marco de trabajo web) puedan ser actualizados o reemplazados 
en el futuro sin comprometer la lógica central del proyecto.
...
La selección de Blazor como tecnología de \textit{frontend} responde a una decisión estratégica basada en la 
eficiencia del ciclo de desarrollo y la optimización de la curva de aprendizaje. Dado que el ecosistema 
tradicional de desarrollo de interfaces web modernas a menudo exige un dominio avanzado de lenguajes y 
bibliotecas basados en JavaScript, Blazor ofrece una alternativa pragmática al permitir la construcción 
de interfaces de usuario altamente interactivas utilizando el lenguaje C\# combinado con componentes HTML. 
Esta característica resultó determinante para el desarrollo del proyecto, ya que posibilitó capitalizar 
los sólidos conocimientos previos de programación en C\# ---empleados en el \textit{backend}--- para 
implementar de manera directa la lógica de presentación del \textit{frontend}. En consecuencia, se 
redujo drásticamente la barrera de entrada técnica y se aceleró el proceso de construcción visual del 
sistema, logrando mantener un código fuente unificado, homogéneo y fuertemente tipado a través de todas las 
...
La Figura~\ref{fig:uc_autenticacion} presenta el diagrama de casos de uso correspondiente a este módulo, donde se distinguen las interacciones del usuario anónimo (registro, confirmación, inicio de sesión y recuperación de credenciales) y las del usuario autenticado (gestión de perfil y contraseña).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{Graphics/uc_autenticacion.png}
    \caption{Diagrama de casos de uso del Módulo de Autenticación y Gestión de Cuenta.}
    \label{fig:uc_autenticacion}
\end{figure}
...
La Figura~\ref{fig:uc_imagenes} muestra el diagrama de casos de uso de este módulo, donde intervienen el usuario anónimo (consulta y previsualización), el colaborador y el administrador (carga y eliminación de imágenes), así como el usuario autorizado para las descargas masivas.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{Graphics/uc_imagenes.png}
    \caption{Diagrama de casos de uso del Módulo de Gestión de Imágenes.}
    \label{fig:uc_imagenes}
\end{figure}
...
La Figura~\ref{fig:uc_usuarios} ilustra las funcionalidades exclusivas del administrador para la gestión integral de las cuentas de usuario del sistema.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{Graphics/uc_usuarios.png}
    \caption{Diagrama de casos de uso del Módulo de Gestión de Usuarios.}
    \label{fig:uc_usuarios}
\end{figure}
...
La Figura~\ref{fig:uc_descargas} representa las interacciones del usuario autenticado (solicitud y consulta de estado) y del administrador (revisión y aprobación/rechazo) en el flujo de autorización de descargas masivas.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{Graphics/uc_descargas.png}
    \caption{Diagrama de casos de uso del Módulo de Solicitudes de Descarga.}
    \label{fig:uc_descargas}
\end{figure}
...
Los módulos de Estadísticas, Instituciones y Metadatos, dado su carácter complementario y reducido número de casos de uso, se representan conjuntamente en la Figura~\ref{fig:uc_estadisticas}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{Graphics/uc_estadisticas.png}
    \caption{Diagrama de casos de uso de los Módulos de Estadísticas, Instituciones y Metadatos.}
    \label{fig:uc_estadisticas}
\end{figure}
...
el registro atómico y definitivo de la entidad principal. La Figura~\ref{fig:flujo_subida} 
detalla este proceso mediante un flujograma.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.65\textwidth]{Graphics/flujo_subida.png}
    \caption{Flujograma del proceso de subida e integración de imágenes.}
    \label{fig:flujo_subida}
\end{figure}
...
ZIP. La Figura~\ref{fig:flujo_descarga} presenta el flujograma correspondiente a este proceso.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.65\textwidth]{Graphics/flujo_descarga.png}
    \caption{Flujograma del proceso de descarga y exportación masiva.}
    \label{fig:flujo_descarga}
\end{figure}
...
\subsubsection{Diagrama del Esquema Relacional}

La Figura~\ref{fig:er_diagram} presenta el diagrama entidad-relación del esquema de base de datos, donde se visualizan las tablas principales del dominio clínico, las tablas estándar del marco de identidad y las relaciones de clave foránea que las conectan. Para mantener la legibilidad del diagrama, las tablas de dominio muestran únicamente sus campos de identificación y claves foráneas, mientras que los campos clínicos detallados se documentan exhaustivamente en las tablas precedentes.

Es importante señalar que el campo \texttt{ContributorId} de la tabla \texttt{Images} almacena el identificador del usuario que subió la imagen desde una aplicación cliente externa al sistema; por consiguiente, \textbf{no constituye una clave foránea} hacia la tabla \texttt{Users} de esta base de datos, sino una referencia de trazabilidad hacia el sistema externo de captura que originó la imagen.

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    % Estilo para tablas de dominio
    entity/.style={
        rectangle split,
        rectangle split parts=2,
        rectangle split part fill={blue!25, white},
        draw=blue!60,
        thick,
        rounded corners=2pt,
        text width=6.2cm,
    ...
    % Estilo para tablas de identidad
    identity/.style={
        rectangle split,
        rectangle split parts=2,
    ...
    % Estilo para tablas auxiliares compactas
    auxiliary/.style={
        rectangle,
        draw=gray!50,
        fill=gray!8,
...
% === TABLA IMAGES (izquierda) ===
\node[entity, text width=7.5cm] (images) {
    \textbf{Images} \hfill \texttt{DermaImg}
    \nodepart{second}
    \underline{\texttt{Id}} \hfill \texttt{uuid PK} \\
    \texttt{PublicId} \hfill \texttt{varchar(30) UNIQUE} \\
    \texttt{FileName} \hfill \texttt{varchar(500)} \\
    \texttt{FilePath} \hfill \texttt{varchar(1000)} \\
    \texttt{ContentType} \hfill \texttt{varchar(100)} \\
    \texttt{FileSize} \hfill \texttt{bigint} \\
    \texttt{IsPublic} \hfill \texttt{boolean} \\
    \texttt{IsDeleted} \hfill \texttt{boolean} \\
    \texttt{CreatedAt} \hfill \texttt{timestamp} \\
    \texttt{ContributorId} \hfill \texttt{uuid (externo)} \\[3pt]
    \textit{+ 30 campos clínicos}\\
    \textit{~~(demográficos, diagnósticos,}\\
    \textit{~~histológicos, institucionales)}
};

% === TABLA USERS (derecha) ===
\node[entity, right=4cm of images, text width=6.8cm] (users) {
    \textbf{Users} \hfill \texttt{User}
    \nodepart{second}
    \underline{\texttt{Id}} \hfill \texttt{uuid PK} \\
    \texttt{FirstName} \hfill \texttt{varchar(100)} \\
    \texttt{LastName} \hfill \texttt{varchar(100)} \\
    \texttt{Email} \hfill \texttt{varchar(256)} \\
    \texttt{UserName} \hfill \texttt{varchar(256)} \\
    \texttt{PasswordHash} \hfill \texttt{text} \\
    \texttt{EmailConfirmed} \hfill \texttt{boolean} \\
    \texttt{IsActive} \hfill \texttt{boolean} \\
    \texttt{IsDeleted} \hfill \texttt{boolean} \\
    \texttt{CreatedAt} \hfill \texttt{timestamp} \\
    \texttt{SecurityStamp} \hfill \texttt{text} \\
    \textit{+ campos de Identity}
};

% === TABLA DOWNLOAD REQUESTS (debajo de Users) ===
\node[entity, below=2cm of users, text width=6.8cm] (downloads) {
    \textbf{DownloadRequests} \hfill \texttt{DownloadRequest}
    \nodepart{second}
    \underline{\texttt{Id}} \hfill \texttt{uuid PK} \\
    \texttt{UserId} \hfill \texttt{uuid FK $\rightarrow$ Users} \\
    \texttt{Reason} \hfill \texttt{varchar(2000)} \\
    \texttt{Institution} \hfill \texttt{varchar(500)} \\
    \texttt{Status} \hfill \texttt{varchar(50)} \\
    \texttt{ReviewedById} \hfill \texttt{uuid FK $\rightarrow$ Users} \\
    \texttt{ReviewedAt} \hfill \texttt{timestamp} \\
    \texttt{CreatedAt} \hfill \texttt{timestamp} \\
    \texttt{IsDeleted} \hfill \texttt{boolean}
};

% === TABLA ROLES (derecha-arriba) ===
\node[identity, above right=0.5cm and 1.5cm of users.north east, text width=5cm] (roles) {
    \textbf{Roles} \hfill \texttt{IdentityRole}
    \nodepart{second}
    \underline{\texttt{Id}} \hfill \texttt{uuid PK} \\
    \texttt{Name} \hfill \texttt{varchar(256)} \\
    \texttt{NormalizedName} \hfill \texttt{varchar(256)} \\
    \texttt{ConcurrencyStamp} \hfill \texttt{text}
};

% === TABLA USERROLES (pivote) ===
\node[identity, below=1.2cm of roles, text width=5cm] (userroles) {
    \textbf{UserRoles} \hfill \textit{N:M Pivote}
    \nodepart{second}
    \underline{\texttt{UserId}} \hfill \texttt{uuid FK $\rightarrow$ Users} \\
    \underline{\texttt{RoleId}} \hfill \texttt{uuid FK $\rightarrow$ Roles}
};

% === TABLAS AUXILIARES (compactas) ===
\node[auxiliary, below=1.2cm of userroles] (userclaims) {\textbf{UserClaims}\\\texttt{FK $\rightarrow$ Users}};
\node[auxiliary, below=0.6cm of userclaims] (userlogins) {\textbf{UserLogins}\\\texttt{FK $\rightarrow$ Users}};
\node[auxiliary, below=0.6cm of userlogins] (usertokens) {\textbf{UserTokens}\\\texttt{FK $\rightarrow$ Users}};
\node[auxiliary, below=0.6cm of usertokens] (roleclaims) {\textbf{RoleClaims}\\\texttt{FK $\rightarrow$ Roles}};

% === NOTA sobre ContributorId (debajo de Images) ===
\node[below=0.8cm of images.south, anchor=north, text width=7.5cm, font=\scriptsize\itshape, 
      draw=orange!60, fill=orange!5, rounded corners=2pt, inner sep=5pt] (extid_note) {
    \texttt{ContributorId} es un identificador externo proveniente del sistema de captura de imágenes. No establece relación de clave foránea con la tabla \texttt{Users} de esta base de datos.
};

% === RELACIONES (flechas FK) ===

% DownloadRequests.UserId → Users.Id (flecha izquierda)
\draw[fk] ([xshift=-1cm]downloads.north)
    -- node[relabel, left, pos=0.5] {UserId}
    ([xshift=-1cm]users.south);

% DownloadRequests.ReviewedById → Users.Id (flecha derecha)
\draw[fk] ([xshift=1cm]downloads.north)
    -- node[relabel, right, pos=0.5] {ReviewedById}
    ([xshift=1cm]users.south);

% UserRoles.UserId → Users.Id
\draw[fkdashed] (userroles.west) -- ++(-0.5cm,0) |- node[relabel, above, pos=0.25] {UserId} ([yshift=-0.3cm]users.east);

% UserRoles.RoleId → Roles.Id
\draw[fkdashed] (userroles.north) -- node[relabel, right, pos=0.5] {RoleId} (roles.south);

% Auxiliary → Users
\draw[fkdashed] (userclaims.west) -- ++(-0.8cm,0) |- ([yshift=-0.9cm]users.east);
\draw[fkdashed] (userlogins.west) -- ++(-0.8cm,0) |- ([yshift=-1.5cm]users.east);
\draw[fkdashed] (usertokens.west) -- ++(-0.8cm,0) |- ([yshift=-2.1cm]users.east);

% RoleClaims → Roles
\draw[fkdashed] (roleclaims.north) -- ++(0, 0.3cm) -| ([xshift=0.3cm]roles.south);

% === LEYENDA ===
\node[below=0.5cm of extid_note.south west, anchor=north west, font=\footnotesize] (legend) {
    \begin{tabular}{@{}cl@{}}
        \tikz\draw[fill=blue!25, draw=blue!60, thick, rounded corners=2pt] (0,0) rectangle (0.4,0.25); & Tablas del dominio clínico \\
        \tikz\draw[fill=gray!20, draw=gray!60, thick, rounded corners=2pt] (0,0) rectangle (0.4,0.25); & Tablas de ASP.NET Core Identity \\
        \tikz\draw[-Stealth, thick, color=blue!60!black] (0,0.12) -- (0.6,0.12); & Clave foránea de dominio \\
        \tikz\draw[-Stealth, thick, dashed, color=gray!60!black] (0,0.12) -- (0.6,0.12); & Clave foránea de Identity \\
        \underline{Campo} & Clave primaria (PK) \\
    \end{tabular}
};

\end{tikzpicture}%
}
\caption{Diagrama entidad-relación del esquema de base de datos de DermaUH.}
...
La Tabla~\ref{tab:comparativa-propuesta} retoma la comparativa de los repositorios internacionales analizados, incorporando las características del esquema de base de datos propuesto para DermaUH, lo que evidencia su posicionamiento estratégico.

\begin{table}[htbp]
\centering
\small
\caption{Comparativa de repositorios internacionales incluyendo la propuesta de DermaUH.}\label{tab:comparativa-propuesta}
...
HAM10000 & 10\,015 & Principalmente I--III & Alta confirmación histopatológica. Edad, sexo y localización anatómica. \\
PAD-UFES-20 & 2\,298 & I--VI (sesgo europeo local) & Capturas móviles. 26 variables clínicas y socioeconómicas. \\
BCN20000 & 18\,946 & Principalmente I--III & Entorno clínico real con ruido. Incluye categoría fuera de distribución (OOD). \\
Derm7pt & >2\,000 & Principalmente I--III & Mapeo con \textit{Checklist} de 7 Puntos. Ideal para Inteligencia Artificial Explicable (XAI). \\
\midrule
\textbf{DermaUH (Propuesta)} & \textbf{En crec.} & \textbf{I--VI (Alta representatividad III--V)} & \textbf{Alineado a ISIC. Variables demográficas, epidemiológicas e histológicas específicas de melanoma. Enfoque en equidad fenotípica local.} \\
\bottomrule
\end{tabular}
\end{table}
